 ...
 */
 class RadialControls extends MenuControls {
    ...
    static onClick(mouseEvent) {
        ...
        if (!(visibleButtonObservable.hasOwnProperty("_observers")) || !(visibleButtonObservable._observers instanceof Array) || visibleButtonObservable._observers.length == 0) {
            return 0;
        }
        // if the button has a callback
        if (!(visibleButtonObservable._observers[0].hasOwnProperty("callback")) || typeof visibleButtonObservable._observers[0].callback != "function") {
            return 0;
        }
